...
fn test_handler_accepts_object_for_struct_input() {
    ...
    assert!(result.is_ok(), "Handler should accept object for struct input");
...
fn test_handler_rejects_null_for_struct_input() {
    ...
    assert!(result.is_err(), "Handler should reject null for struct input");
}
...
fn test_handler_accepts_empty_object_for_optional_fields() {
    ...
    assert!(result.is_ok(), "Handler should accept {{}} for struct with optional fields");
...
async fn test_struct_handler_with_valid_input() {
    let result = simulate_handler_call(
        json!({"name": "Alice"}),
        |input: TestInput| format!("Hello, {}", input.name),
    ).await;
    assert!(result.is_ok());
