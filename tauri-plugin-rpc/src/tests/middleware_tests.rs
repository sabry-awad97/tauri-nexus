...
use crate::{
    Context, RpcError, RpcResult, Router,
...
fn create_logging_middleware(
    name: String,
) -> impl Fn(Context<TestContext>, Request, Next<TestContext>) -> std::pin::Pin<Box<dyn std::future::Future<Output = RpcResult<Response>> + Send>> + Send + Sync + 'static {
...
async fn test_compiled_router_middleware_order() {
    ...
    let log = test_ctx.execution_log.lock().await;
    let expected = vec![
        "M1_enter", "M2_enter", "M3_enter",
        "handler",
        "M3_exit", "M2_exit", "M1_exit"
    ];
...
async fn test_compiled_router_early_return() {
    ...
    let router = Router::new()
        .context(test_ctx.clone())
        .middleware(create_logging_middleware("M1".to_string()))
        .middleware(create_early_return_middleware("M2".to_string(), early_value.clone()))
        .middleware(create_logging_middleware("M3".to_string()))
        .query("test", test_handler)
        .compile();
...
async fn test_compiled_router_error_propagation() {
    ...
    let router = Router::new()
        .context(test_ctx.clone())
        .middleware(create_logging_middleware("M1".to_string()))
        .middleware(create_error_middleware("M2".to_string(), "test error".to_string()))
        .middleware(create_logging_middleware("M3".to_string()))
        .query("test", test_handler)
        .compile();
