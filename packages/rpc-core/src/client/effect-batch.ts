...
import { makeCallError, parseEffectError, toPublicError } from "../internal/effect-errors";
...
> {
  ...
  executeEffect(
    ...
    return pipe(
      ...
      Effect.map((response) => new EffectBatchResponseWrapper<TOutputMap>(response)),
    );
  }
  ...
  validateEffect(): Effect.Effect<void, RpcEffectError> {
    return Effect.gen(function* (this: EffectBatchBuilder<TContract, TOutputMap>) {
...
export class EffectBatchResponseWrapper<TOutputMap extends OutputTypeMap> {
  ...
  getResultEffect<TId extends keyof TOutputMap & string>(
    ...
    return Effect.gen(function* (this: EffectBatchResponseWrapper<TOutputMap>) {
      ...
      if (result.error) {
        return yield* Effect.fail(
          makeCallError(result.error.code, result.error.message, result.error.details),
        );
      }
