...
const primitiveTypeArb = fc.constantFrom('string', 'number', 'integer', 'boolean', 'null');
...
const primitiveSchemaArb: fc.Arbitrary<TypeSchema> = fc.record({
  ...
  example: fc.option(fc.oneof(fc.string(), fc.integer(), fc.boolean()), { nil: undefined }),
  ...
  format: fc.option(fc.constantFrom('email', 'uuid', 'date-time'), { nil: undefined }),
  nullable: fc.option(fc.boolean(), { nil: undefined }),
  enum: fc.option(fc.array(fc.string({ minLength: 1, maxLength: 10 }), { minLength: 1, maxLength: 5 }), { nil: undefined }),
});

/** Generate a property name */
const propertyNameArb = fc.string({ minLength: 1, maxLength: 20 }).filter(s => /^[a-zA-Z][a-zA-Z0-9_]*$/.test(s));

/** Generate an object type schema with properties */
const objectSchemaArb: fc.Arbitrary<TypeSchema> = fc
  .array(fc.tuple(propertyNameArb, primitiveSchemaArb), { minLength: 1, maxLength: 5 })
  .chain((props) => {
...
const arraySchemaArb: fc.Arbitrary<TypeSchema> = primitiveSchemaArb.map((itemSchema) => ({
  type: 'array',
  items: itemSchema,
  minLength: undefined,
  maxLength: undefined,
...
const anySchemaArb = fc.oneof(primitiveSchemaArb, objectSchemaArb, arraySchemaArb);
      ...
      fc.property(objectSchemaArb, (schema) => {
        ...
        if (schema.properties) {
          for (const propName of Object.keys(schema.properties)) {
            const propElement = container.querySelector(`[data-testid="property-${propName}"]`);
      ...
      fc.property(objectSchemaArb, (schema) => {
        ...
        if (schema.properties) {
          for (const propName of Object.keys(schema.properties)) {
            const propElement = container.querySelector(`[data-testid="property-${propName}"]`);
            if (propElement) {
              const hasRequiredIndicator = propElement.querySelector('.type-required') !== null;
  ...
  it('displays minLength constraint when present', () => {
    fc.assert(
      fc.property(
        ...
        (schema) => {
          const { container } = render(<TypeRenderer schema={schema} />);
          expect(container.textContent).toContain(`minLength: ${schema.minLength}`);
        }
      ),
      { numRuns: 30 }
    );
  ...
  it('displays maxLength constraint when present', () => {
    fc.assert(
      fc.property(
        ...
        (schema) => {
          const { container } = render(<TypeRenderer schema={schema} />);
          expect(container.textContent).toContain(`maxLength: ${schema.maxLength}`);
        }
      ),
      { numRuns: 30 }
    );
  ...
  it('displays example when present and showExamples is true', () => {
    fc.assert(
      fc.property(
        ...
        (schema) => {
          const { container } = render(<TypeRenderer schema={schema} showExamples={true} />);
          expect(container.textContent).toContain('e.g.');
          expect(container.textContent).toContain(JSON.stringify(schema.example));
        }
      ),
      { numRuns: 30 }
    );
  ...
  it('hides example when showExamples is false', () => {
    fc.assert(
      fc.property(
        primitiveSchemaArb.filter(s => s.example !== undefined),
        (schema) => {
          const { container } = render(<TypeRenderer schema={schema} showExamples={false} />);
