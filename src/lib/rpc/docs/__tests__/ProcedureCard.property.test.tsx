...
describe('Property 2: Procedure Card Displays Required Information', () => {
    ...
    fc.assert(
      fc.property(procedurePathArb, procedureSchemaArb, (path, schema) => {
        ...
        const { container } = render(
          <ProcedureCard path={path} schema={schema} expanded={false} onToggle={onToggle} />
        );
        
        const pathElement = container.querySelector('[data-testid="procedure-path"]');
    ...
    fc.assert(
      fc.property(
        ...
        (path, schema) => {
          ...
          const deprecatedIndicator = container.querySelector('[data-testid="deprecated-indicator"]');
  ...
  it('calls onToggle when header is clicked', () => {
    fc.assert(
      fc.property(procedurePathArb, procedureSchemaArb, fc.boolean(), (path, schema, expanded) => {
        const onToggle = vi.fn();
        const { container } = render(
          <ProcedureCard path={path} schema={schema} expanded={expanded} onToggle={onToggle} />
        );
...
describe('Property 7: Expanded Procedure Shows Additional Details', () => {
  it('shows description when expanded and description exists', () => {
    fc.assert(
      fc.property(
        procedurePathArb,
        procedureSchemaArb.filter(s => s.description !== undefined && s.description.length > 0),
        (path, schema) => {
          const onToggle = vi.fn();
          const { container } = render(
            <ProcedureCard path={path} schema={schema} expanded={true} onToggle={onToggle} />
          );
          
          const descElement = container.querySelector('[data-testid="description"]');
    ...
    fc.assert(
      fc.property(
        ...
        (path, schema) => {
          ...
          const inputElement = container.querySelector('[data-testid="input-schema"]');
    ...
    fc.assert(
      fc.property(
        ...
        (path, schema) => {
          ...
          const outputElement = container.querySelector('[data-testid="output-schema"]');
    ...
    fc.assert(
      fc.property(
        ...
        (path, schema) => {
          ...
          const metadataElement = container.querySelector('[data-testid="metadata"]');
  ...
  it('hides details when not expanded', () => {
    fc.assert(
      fc.property(procedurePathArb, procedureSchemaArb, (path, schema) => {
        ...
        const detailsElement = container.querySelector('[data-testid="procedure-details"]');
