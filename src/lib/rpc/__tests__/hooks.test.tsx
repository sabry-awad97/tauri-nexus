...
import type { RpcError, ContractRouter, RouterClient, EventIterator } from '../types';

// =============================================================================
// Test Utilities
// =============================================================================

function createMockEventIterator<T>(events: T[], error?: RpcError): EventIterator<T> {
  let index = 0;
  let returned = false;
...
describe('useSubscription', () => {
  it('should connect and receive events', async () => {
    const events = [1, 2, 3];
    const subscribeFn = vi.fn().mockResolvedValue(createMockEventIterator(events));
  ...
  it('should handle errors', async () => {
    ...
    const subscribeFn = vi.fn().mockResolvedValue(createMockEventIterator([], error));
...
describe('createHooks', () => {
  interface TestContract extends ContractRouter {
    ...
    createUser: { type: 'mutation'; input: { name: string }; output: { id: number } };
      ...
      fc.asyncProperty(
        ...
        async (shouldSucceed, errorMessage) => {
          const queryFn = shouldSucceed
            ...
            : vi.fn().mockRejectedValue({ code: 'ERROR', message: errorMessage || 'Error' });
