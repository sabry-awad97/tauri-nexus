    ...
    fc.assert(
      fc.property(
        ...
        (attempt, baseDelay) => {
          ...
          const delay1 = calculateBackoff(attempt, baseDelay, maxDelay, false);
          const delay2 = calculateBackoff(attempt + 1, baseDelay, maxDelay, false);
...
describe('withRetry()', () => {
  ...
  it('should retry on retryable errors', async () => {
    const fn = vi.fn()
      .mockRejectedValueOnce({ code: 'INTERNAL_ERROR', message: 'Error' } as RpcError)
      .mockRejectedValueOnce({ code: 'TIMEOUT', message: 'Timeout' } as RpcError)
