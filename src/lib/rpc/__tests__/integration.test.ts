...
interface ApiContract {
  ...
  notifications: {
    subscribe: { type: 'subscription'; input: { userId: number }; output: { message: string } };
  };
}
...
describe('Integration: Complete RPC Workflow', () => {
  describe('Basic Client Usage', () => {
    it('should perform a complete CRUD workflow', async () => {
      ...
      const created = await (client as any).user.create({ name: 'John', email: 'john@example.com' });
...
describe('Integration: Type Safety Verification', () => {
  it('should enforce correct input types', async () => {
    ...
    mockInvoke.mockResolvedValue({ id: 1, name: 'Test', email: 'test@example.com' });
    ...
    await (client as any).user.create({ name: 'Test', email: 'test@example.com' });
  ...
  it('should infer correct output types', async () => {
    ...
    mockInvoke.mockResolvedValueOnce({ id: 1, name: 'Test', email: 'test@example.com' });
