...
async function subscriptionExamples() {
  ...
  const stockStream = await subscribe<StockPrice>("stocks.live", {
    symbols: ["AAPL", "GOOGL"],
  });
  ...
  const chatStream = await subscribe<ChatMessage>("chat.messages", {
    roomId: "general",
  });
...
export function UserProfile({ userId }: { userId: number }) {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(() => rpc.user.get({ id: userId }), [userId]);
...
export function ChatRoom({ roomId }: { roomId: string }) {
  ...
  // ✅ Type-safe mutation
  const sendMessage = useMutation<SendMessageInput, ChatMessage>((input) =>
    rpc.chat.send(input),
  );
